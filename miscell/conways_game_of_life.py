...
def printGrid (grid, stdoutOrStdScr = False, stdscr = 0, y_coordinate = 0, x_coordinate = 0):
...
def createNextStateGrid():
    rows, columns = calcRowsAndColumns(grid)
    nextStateGrid = [ [ ' ' for c in range(columns) ] for r in range(rows)]

    for r in range(rows):
        for c in range(columns):
            nextStateGrid[r][c] = cellNextState(r,c)
...
def draw_menu(stdscr):
    ...
    print("max number of rows:columns allowed are: " + str(rows) + ":" + str(columns))
    if rows > height - 2 or columns > width - 1:
        print("max number of rows:columns allowed are: " + str(rows) + ":" + str(columns))
        return
    ...
    while (k != ord('q')):
        ...
        printGrid(grid, True, stdscr, y_coordinate, x_coordinate)
        stdscr.attron(curses.color_pair(1))
...
def main():
    ...
    for i in range(cells_alive_initially):
